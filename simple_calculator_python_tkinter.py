...
class Calculator:
    ...
    def bind_keys(self):
        self.root.bind('<F1>', self.open_text_window)

    def open_text_window(self, event):
        new_window = tk.Toplevel(self.root)
        new_window.title("Автор")
        new_window.geometry("600x150")
        label = tk.Label(new_window, text="Филимонов Максим Алексеевич БИСО-03-23", font=("Arial", 14))
        label.pack(pady=50)
    ...
    def on_button_click(self, button):
        ...
        if button == "C" or button == "CE":
            ...
            self.result_var.set(current_text + ",")
        elif button in "%÷+-,×" and current_text != "Ошибка":
            if current_text[-1] not in "%÷+-×,":
                self.result_var.set(current_text + button)
            else:
                self.result_var.set(current_text[:-1] + button)
        else:
            if current_text != "Ошибка":
                if current_text == "0":
                    self.result_var.set(button)
